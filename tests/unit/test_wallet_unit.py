...
from scripts.deploy import deploy, TESTNETS
import pytest
...
def test_owners():
    if network.show_active() in TESTNETS:
        ...
        account = get_account()
        multi_sig_wallet = deploy()

        assert multi_sig_wallet.owners(0) == account
        assert multi_sig_wallet.owners(1) == accounts[1]
        assert multi_sig_wallet.owners(2) == accounts[2]
        assert len(multi_sig_wallet.getOwners()) == 3
        assert multi_sig_wallet.getOwners() == (account, accounts[1], accounts[2])
...
def test_revoke_confirmation():
    ...
    # Test revoking confirmation
    assert multi_sig_wallet_not_executed.getTransaction(0)[4] == 2
    multi_sig_wallet_not_executed.revokeConfirmation(0, {"from": account})
    assert multi_sig_wallet_not_executed.getTransaction(0)[4] == 1
